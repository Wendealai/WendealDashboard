  ...
  return (
    <Layout className='sparkery-saas-layout'>
      ...
      >
        ...
        {!collapsed ? (
          <div className='sparkery-saas-quick-panels'>
            <div className='sparkery-saas-quick-panel'>
              <Text className='sparkery-saas-quick-title'>
                {t('sparkery.saas.labels.pinnedModules', {
                  defaultValue: 'Pinned Modules',
                })}
              </Text>
              <Space wrap size={[6, 6]}>
                {pinnedModules.length > 0 ? (
                  ...
                  <Text type='secondary' className='sparkery-saas-quick-empty'>
                    {t('sparkery.saas.empty.noPinned', {
                      defaultValue: 'No pinned modules',
                    })}
                  </Text>
                )}
              </Space>
            ...
            <div className='sparkery-saas-quick-panel'>
              <Text className='sparkery-saas-quick-title'>
                {t('sparkery.saas.labels.favorites', {
              ...
              <Space wrap size={[6, 6]}>
                {favoriteModules.length > 0 ? (
                  ...
                  <Text type='secondary' className='sparkery-saas-quick-empty'>
                    {t('sparkery.saas.empty.noFavorites', {
            ...
            <div className='sparkery-saas-quick-panel'>
              <Text className='sparkery-saas-quick-title'>
                {t('sparkery.saas.labels.recentModules', {
                  defaultValue: 'Recent Modules',
                })}
              </Text>
              <Space wrap size={[6, 6]}>
                {recentModules.length > 0 ? (
                  ...
                  <Text type='secondary' className='sparkery-saas-quick-empty'>
                    {t('sparkery.saas.empty.noRecent', {
                      defaultValue: 'No recent modules',
                    })}
                  </Text>
                )}
              </Space>
      ...
      <Layout className='sparkery-saas-main'>
        ...
        <Content className='sparkery-saas-content'>
          <div className='sparkery-saas-ops-strip'>
            {showOpsKpi ? (
              <Card
                size='small'
                className='sparkery-saas-ops-card sparkery-saas-ops-kpi-card'
                title={t('sparkery.saas.ops.kpis', {
                  defaultValue: 'Module KPIs',
                })}
                extra={
                  <Button size='small' type='text' icon={<ReloadOutlined />} onClick={refreshModuleData}>
                    {t('sparkery.saas.actions.refresh', { defaultValue: 'Refresh' })}
                  </Button>
                }
              >
                <div className='sparkery-saas-kpi-grid'>
                  {moduleKpis.map(kpi => (
                    <div key={kpi.id} className='sparkery-saas-kpi-item'>
                      <Text className='sparkery-saas-kpi-label'>{kpi.label}</Text>
                      <Text className='sparkery-saas-kpi-value'>{kpi.value}</Text>
                      <Tag
                        className={
                          kpi.delta >= 0
                            ? 'sparkery-saas-kpi-delta sparkery-saas-kpi-delta-positive'
                            : 'sparkery-saas-kpi-delta sparkery-saas-kpi-delta-negative'
                        }
                      >
                        <Space size={4}>
                          {kpi.delta >= 0 ? <RiseOutlined /> : <ExclamationCircleOutlined />}
            ...
            <Card
              size='small'
              className='sparkery-saas-ops-card sparkery-saas-ops-health-card'
              title={t('sparkery.saas.ops.systemHealth', {
                defaultValue: 'System Health',
              })}
              extra={
                <Tag className={`sparkery-saas-chip sparkery-saas-chip--${systemHealth.tone}`}>
                  {systemHealth.label}
                </Tag>
              }
            >
              <Space direction='vertical' size={6} className='sparkery-saas-health-stack'>
                <Text>
                  {t('sparkery.saas.ops.lastSync', {
                    defaultValue: 'Last Sync',
                  })}
                  : {formatMinutesAgo(lastSyncAt)}
                </Text>
                <Text>
                  {t('sparkery.saas.ops.dataFreshness', {
                    defaultValue: 'Data Freshness',
                  })}
                  : {moduleFreshnessMinutes}m
                </Text>
                <Progress
                  percent={freshnessScore}
                  size='small'
                  status={freshnessScore >= 70 ? 'success' : freshnessScore >= 40 ? 'normal' : 'exception'}
                  showInfo={false}
                />
              </Space>
            </Card>
            ...
            <Card
              size='small'
              className='sparkery-saas-ops-card sparkery-saas-ops-focus-card'
              title={t('sparkery.saas.ops.todayFocus', {
                defaultValue: 'Today Focus',
              })}
              extra={<Badge count={todayFocusCount} color='#1677ff' />}
            >
              <Text type='secondary'>
                {t('sparkery.saas.ops.todayFocusHint', {
                  defaultValue: 'Critical alerts and high-priority tasks requiring review.',
                })}
              </Text>
              <div className='sparkery-saas-inline-actions'>
                <Button size='small' onClick={() => setAlertCenterOpen(true)} icon={<AlertOutlined />}>
                  {t('sparkery.saas.actions.openAlerts', { defaultValue: 'Open Alerts' })}
                </Button>
                <Button size='small' onClick={() => setActivityDrawerOpen(true)} icon={<HistoryOutlined />}>
                  {t('sparkery.saas.actions.openActivity', { defaultValue: 'Open Activity' })}
                </Button>
              </div>
            </Card>
            ...
            <Card
              size='small'
              className='sparkery-saas-ops-card sparkery-saas-ops-tasks-card'
              title={t('sparkery.saas.ops.myTasks', {
                defaultValue: 'My Tasks',
              })}
            >
              <List
                size='small'
                dataSource={outstandingTasks.slice(0, 4)}
                locale={{
                  emptyText: t('sparkery.saas.empty.noTasks', {
                    defaultValue: 'No pending tasks',
                  }),
                }}
                renderItem={task => (
                  <List.Item
                    actions={[
                      <Button key={`task-done-${task.id}`} type='link' size='small' onClick={() => toggleTaskDone(task.id)}>
                        {t('sparkery.saas.actions.done', { defaultValue: 'Done' })}
                      </Button>,
                    ]}
                  >
                    <Space direction='vertical' size={0}>
                      <Text>{task.title}</Text>
                      <Text type='secondary'>{`${task.dueLabel} · ${task.priority}`}</Text>
                    </Space>
                  </List.Item>
                )}
              />
            ...
            <Card
              size='small'
              className='sparkery-saas-ops-card sparkery-saas-ops-jobs-card'
              title={t('sparkery.saas.ops.scheduledJobs', {
                defaultValue: 'Scheduled Jobs',
              })}
            >
              <div className='sparkery-saas-jobs-summary'>
                <Tag>{`Running ${scheduledJobs.running}`}</Tag>
                <Tag>{`Queued ${scheduledJobs.queued}`}</Tag>
                <Tag color={scheduledJobs.failed > 0 ? 'error' : 'success'}>
                  {`Failed ${scheduledJobs.failed}`}
                </Tag>
              </div>
              <Text type='secondary'>{`Success ${scheduledJobs.successRate}%`}</Text>
              <div className='sparkery-saas-inline-actions'>
                <Button
                  size='small'
                  icon={<ReloadOutlined />}
                  disabled={scheduledJobs.failed <= 0}
                  onClick={retryFailedJobs}
                >
                  {t('sparkery.saas.actions.retryFailed', {
            ...
            <Card
              size='small'
              className='sparkery-saas-ops-card sparkery-saas-ops-create-card'
              title={t('sparkery.saas.ops.quickCreate', {
              ...
              <Space wrap>
                {quickCreateItems.map(item => (
                  <Button
                    key={item.id}
                    size='small'
                    onClick={() => executeModuleNavigation(item.targetPath)}
                  >
                    {item.label}
                  </Button>
                ))}
            ...
            {showApprovals ? (
              <Card
                size='small'
                className='sparkery-saas-ops-card sparkery-saas-ops-approvals-card'
                title={t('sparkery.saas.ops.pendingApprovals', {
                  ...
                  renderItem={approval => (
                    <List.Item>
                      <Space>
                        ...
                        <Tag color='processing'>{approval.count}</Tag>
                      </Space>
                    </List.Item>
                  )}
                />
            ...
            {showWorkloadHeatmap ? (
              <Card
                size='small'
                className='sparkery-saas-ops-card sparkery-saas-ops-heatmap-card'
                title={t('sparkery.saas.ops.workloadHeatmap', {
                  defaultValue: 'Ops Workload',
                })}
              >
                <div className='sparkery-saas-heatmap'>
                  {WORKLOAD_HEATMAP.map(entry => (
                    <div key={entry.day} className='sparkery-saas-heatmap-col'>
                      <div
                        className='sparkery-saas-heatmap-bar'
                        style={{ height: `${Math.round(18 + entry.load * 46)}px` }}
                      />
                      <Text className='sparkery-saas-heatmap-label'>{entry.day}</Text>
                    </div>
                  ))}
                </div>
              </Card>
