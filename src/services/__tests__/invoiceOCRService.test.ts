  ...
  describe('工作流管理', () => {
    describe('createWorkflow', () => {
      it('应该成功创建 Invoice OCR 工作流', async () => {
        ...
        expect(mockApi.post).toHaveBeenCalledWith('/api/invoice-ocr', {
          ...request,
          settings: {
            ...DEFAULT_INVOICE_OCR_SETTINGS,
            ...request.settings,
    ...
    describe('toggleWorkflow', () => {
      it('应该成功切换工作流状态', async () => {
        ...
        expect(mockApi.patch).toHaveBeenCalledWith(
          `/api/invoice-ocr/${workflowId}/toggle`,
          { enabled }
        );
        expect(result).toEqual(mockWorkflow);
      });
  ...
  describe('文件处理', () => {
    describe('uploadFiles', () => {
      it('应该成功上传文件进行批处理', async () => {
        ...
        expect(mockApi.post).toHaveBeenCalledWith(
          `/api/invoice-ocr/${request.workflowId}/upload`,
          expect.any(FormData),
          {
            headers: {
    ...
    describe('uploadSingleFile', () => {
      it('应该成功上传单个文件进行处理', async () => {
        ...
        expect(mockApi.post).toHaveBeenCalledWith(
          `/api/invoice-ocr/${workflowId}/upload-single`,
          expect.any(FormData),
          {
            headers: {
              'Content-Type': 'multipart/form-data',
  ...
  describe('结果管理', () => {
    describe('getResult', () => {
      it('应该成功获取处理结果', async () => {
        ...
        expect(mockApi.get).toHaveBeenCalledWith(
          `/api/invoice-ocr/${workflowId}/results/${resultId}`
        );
        expect(result).toEqual(mockResult);
      });
    });
    ...
    describe('deleteResults', () => {
      it('应该成功批量删除处理结果', async () => {
        ...
        expect(mockApi.delete).toHaveBeenCalledWith(
          `/api/invoice-ocr/${workflowId}/results/batch`,
          {
            data: { resultIds },
          }
        );
  ...
  describe('批处理任务管理', () => {
    describe('getBatchTask', () => {
      it('应该成功获取批处理任务状态', async () => {
        ...
        expect(mockApi.get).toHaveBeenCalledWith(
          `/api/invoice-ocr/${workflowId}/batches/${batchId}`
        );
        expect(result).toEqual(mockBatchTask);
      });
    });
    ...
    describe('cancelBatchTask', () => {
      it('应该成功取消批处理任务', async () => {
        ...
        expect(mockApi.post).toHaveBeenCalledWith(
          `/api/invoice-ocr/${workflowId}/batches/${batchId}/cancel`
        );
      });
    });
    ...
    describe('retryBatchTask', () => {
      it('应该成功重试批处理任务', async () => {
        ...
        expect(mockApi.post).toHaveBeenCalledWith(
          `/api/invoice-ocr/${workflowId}/batches/${batchId}/retry`
        );
        expect(result).toEqual(mockBatchTask);
      });
    });
  ...
  describe('文件下载', () => {
    describe('downloadResult', () => {
      it('应该成功下载处理结果文件', async () => {
        ...
        expect(mockApi.get).toHaveBeenCalledWith(
          `/api/invoice-ocr/${workflowId}/results/${resultId}/download`,
          {
            params: { format },
            responseType: 'blob',
          }
    ...
    describe('downloadResults', () => {
      it('应该成功批量下载结果文件', async () => {
        ...
        expect(mockApi.post).toHaveBeenCalledWith(
          `/api/invoice-ocr/${workflowId}/results/download-batch`,
          {
            resultIds,
            format,
          },
