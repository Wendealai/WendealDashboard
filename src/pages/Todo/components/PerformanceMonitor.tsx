 ...
 * Monitors and reports Todo module performance metrics
 */
...
const PerformanceMonitor: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        console.log('Todo: User left page');
      } else {
        console.log('Todo: User returned to page');
      }
    };
    ...
    const handleIframeLoad = () => {
      ...
      console.log('Todo Performance: Iframe loaded', {
      ...
      if ('performance' in window && 'mark' in window.performance) {
        try {
          window.performance.mark('todo-iframe-loaded');
          window.performance.measure(
            'todo-iframe-load-time',
            'todo-iframe-load-start',
            'todo-iframe-loaded'
          );
        } catch (error) {
          console.warn(
            'Todo Performance: Could not record performance marks:',
            error
          );
        }
      }
    ...
    const handleError = () => {
      ...
      console.warn('Todo Performance: Error occurred', {
    ...
    const handleRetry = () => {
      ...
      console.log('Todo Performance: Retry attempted', {
    ...
    // Listen for custom events from TodoContainer
    window.addEventListener('todo-iframe-load-start', () => {
      metricsRef.current.iframeLoadStart = Date.now();
      if ('performance' in window && 'mark' in window.performance) {
        try {
          window.performance.mark('todo-iframe-load-start');
        } catch (error) {
          console.warn('Todo Performance: Could not record start mark:', error);
        }
      }
    });

    window.addEventListener('todo-iframe-loaded', handleIframeLoad);
    window.addEventListener('todo-error', handleError);
    window.addEventListener('todo-retry', handleRetry);
    ...
    if ('performance' in window && 'getEntriesByType' in window.performance) {
      const observer = new PerformanceObserver(list => {
        ...
        entries.forEach(entry => {
          if (entry.name === 'first-paint') {
            ...
            console.log('Todo Performance: First Paint', {
      ...
      try {
        ...
        console.warn(
          'Todo Performance: Could not observe paint events:',
          error
        );
      }
    ...
    return () => {
      ...
      window.removeEventListener('todo-iframe-loaded', handleIframeLoad);
      window.removeEventListener('todo-error', handleError);
      window.removeEventListener('todo-retry', handleRetry);
    };
  }, []);
  ...
  useEffect(() => {
    return () => {
      ...
      console.log('Todo Performance: Session ended', finalMetrics);

      // In production, send metrics to monitoring service
      if (process.env.NODE_ENV === 'production') {
        // Example: Send to monitoring service
        // monitoringService.track('todo-session-end', finalMetrics);
      }
    };
  }, []);
