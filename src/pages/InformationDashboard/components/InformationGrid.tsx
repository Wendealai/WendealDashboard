 ...
 * 信息类型颜色映射
 */
const getTypeColor = (type: InformationType): string => {
  switch (type) {
    case 'text':
      return 'blue';
    case 'number':
 ...
 * 优先级颜色映射
 */
const getPriorityColor = (priority: InformationPriority): string => {
  switch (priority) {
    case 'urgent':
      return 'red';
    case 'high':
 ...
 * 状态颜色映射
 */
const getStatusColor = (status: InformationStatus): string => {
  switch (status) {
    case 'active':
      return 'success';
    case 'archived':
...
const InformationGrid: React.FC<InformationGridProps> = ({
  ...
  const handleConfirmEdit = useCallback(async () => {
    ...
    try {
      ...
      message.error(t('informationGrid.messages.updateFailed'));
    }
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      try {
        ...
        message.error(t('informationGrid.messages.deleteFailed'));
      }
    },
    [handleSearch]
  );
  ...
  const handleBatchDelete = useCallback(async () => {
    ...
    try {
      ...
      message.error(t('informationGrid.messages.batchDeleteFailed'));
    }
   ...
   * 渲染内容
   */
  const renderContent = (content: string, type: InformationType) => {
    switch (type) {
      case 'url':
        return (
        ...
        <Row gutter={16} style={{ marginBottom: 16 }}>
          ...
          <Col span={4}>
            ...
            >
              ...
              <Option value='image'>{t('informationGrid.types.image')}</Option>
            </Select>
          </Col>
          <Col span={4}>
            <Select
          ...
          <div>
            <Row gutter={16}>
              <Col span={12}>
                ...
                <p>
                  <strong>{t('informationGrid.detailModal.category')}:</strong>{' '}
              ...
              <Col span={12}>
                <p>
                  <strong>{t('informationGrid.detailModal.priority')}:</strong>
                ...
                <p>
                  <strong>{t('informationGrid.detailModal.createdAt')}:</strong>{' '}
                  {new Date(selectedItem.createdAt).toLocaleString()}
                </p>
                <p>
                  <strong>{t('informationGrid.detailModal.updatedAt')}:</strong>{' '}
        ...
        <Form form={editForm} layout='vertical'>
          ...
          >
            <Select>
              ...
              <Option value='draft'>{t('informationGrid.status.draft')}</Option>
            </Select>
          </Form.Item>
          ...
          >
            ...
            >
              <Option value='系统'>{t('informationGrid.tags.system')}</Option>
              <Option value='监控'>
                {t('informationGrid.tags.monitoring')}
              </Option>
              <Option value='用户'>{t('informationGrid.tags.user')}</Option>
              <Option value='产品'>{t('informationGrid.tags.product')}</Option>
              <Option value='数据'>{t('informationGrid.tags.data')}</Option>
            </Select>
          </Form.Item>
        </Form>
      </Modal>
