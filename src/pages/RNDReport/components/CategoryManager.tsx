...
const CategoryManager: React.FC<CategoryManagerProps> = ({
  ...
  const handleDeleteCategory = async (category: Category) => {
    ...
    confirm({
      ...
      content: (
        <div>
          <p>{t('rndReport.category.delete.confirm', { name: category.name })}</p>
          {reportCount > 0 && (
  ...
  const handleFormSubmit = async (values: CategoryFormValues) => {
    ...
    try {
      ...
      if (editingCategory) {
        ...
        const updatedCategory = await service.updateCategory(editingCategory.id, {
          name: values.name,
  ...
  const renderCategoryItem = (category: Category) => {
    const reportCount = categoryStats.get(category.id) || 0;
    const isSystemCategory = category.name === t('rndReport.category.default.uncategorized.name') ||
    ...
    return (
      <List.Item
        ...
        actions={[
          ...
          !isSystemCategory && (
            <Popconfirm
              title={t('rndReport.category.delete.confirm', { name: category.name })}
          ...
          description={
            <Space direction="vertical" size={2}>
              ...
              <Space size={8}>
                ...
                <Text type="secondary" style={{ fontSize: '12px' }}>
                  {t('rndReport.category.created')}: {new Date(category.createdDate).toLocaleDateString()}
                </Text>
              </Space>
            </Space>
          }
          ...
          <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
            ...
            <Card size="small" style={{ minWidth: '120px' }}>
              <Space direction="vertical" size={0} align="center">
                ...
                <Text strong style={{ fontSize: '24px' }}>
                  {categories.filter(cat => (categoryStats.get(cat.id) || 0) > 0).length}
                </Text>
              </Space>
            </Card>
            <Card size="small" style={{ minWidth: '120px' }}>
              <Space direction="vertical" size={0} align="center">
                ...
                <Text strong style={{ fontSize: '24px' }}>
                  {Array.from(categoryStats.values()).reduce((sum, count) => sum + count, 0)}
                </Text>
              </Space>
            </Card>
          </div>
            ...
            <List
              ...
              pagination={{
                ...
                showTotal: (total, range) =>
                  t('rndReport.list.pagination', { range0: range[0], range1: range[1], total }),
              }}
            />
          )}
        </Space>
      ...
      >
        ...
        >
          <Form.Item
            ...
            rules={[
              { required: true, message: t('rndReport.category.validation.nameRequired') },
          ...
          <Form.Item
            name="color"
            label={t('rndReport.category.form.color')}
          >
            <ColorPicker
              disabled={submitting}
              presets={[
